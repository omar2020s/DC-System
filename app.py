...
# On Render use PostgreSQL and set DATABASE_URL automatically from the database.
# Locally the app falls back to SQLite for testing only.
# Database configuration
# Use PostgreSQL on Render for permanent records.
# If DATABASE_URL is missing and a Render Disk exists, SQLite will be stored on /var/data.
def default_database_url():
    if os.path.isdir('/var/data'):
        return 'sqlite:////var/data/cement_dc.db'
...
# Persistent uploaded files configuration
# For permanent uploaded PDFs/drawings on Render, create a Render Disk and set:
# UPLOAD_FOLDER=/var/data/uploads
# Permanent uploaded files configuration
# On Render create a Render Disk mounted at /var/data and set:
# UPLOAD_FOLDER=/var/data/uploads
# If UPLOAD_FOLDER is not set but /var/data exists, the app uses /var/data/uploads automatically.
...
def save_file(file):
    if file and file.filename:
        ...
        if not safe_name:
            return None
        name = datetime.now().strftime('%Y%m%d%H%M%S_%f_') + safe_name
...
def storage_status():
    return {
        ...
        'message': 'For permanent files on Render, mount a Render Disk at /var/data and set UPLOAD_FOLDER=/var/data/uploads.'
    }
