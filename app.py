...
# Render currently may use Python 3.14 by default. This project uses psycopg v3,
# so PostgreSQL URLs must use the SQLAlchemy driver form: postgresql+psycopg://
def default_database_url():
    return 'sqlite:///cement_dc.db'
...
# Files are stored permanently inside the database by default.
# This avoids Render Free disk limitations. UPLOAD_FOLDER is only a fallback for local testing.
...
class MAR(db.Model):
    ...
    remarks = db.Column(db.Text)
    signed_file_link = db.Column(db.String(500))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def save_file(file):
    """Save uploaded files permanently in the database.

    Returning storage_name preserves compatibility with existing templates that
    open files through /uploads/<storage_name>.
    """
    ...
    if not safe_name:
        return None
    storage_name = datetime.now().strftime('%Y%m%d%H%M%S_%f_') + safe_name
    attachment = FileAttachment(
        storage_name=storage_name,
        original_name=safe_name,
        content_type=file.mimetype or 'application/octet-stream',
        data=file.read(),
        uploaded_by=getattr(current_user, 'full_name', None) if current_user.is_authenticated else None,
    )
...
def uploads(filename):
    attachment = FileAttachment.query.filter_by(storage_name=filename).first()
    if attachment:
        return Response(
            attachment.data,
    ...
    # Local fallback for old files stored on disk during development.
...
def storage_status():
    return {
        'database_url_type': 'postgresql' if app.config['SQLALCHEMY_DATABASE_URI'].startswith('postgresql') else 'sqlite',
        'file_storage': 'database',
        'saved_files_in_database': FileAttachment.query.count(),
        'message': 'Files are stored permanently in PostgreSQL database. No Render Disk is required.'
    }
...
def mar_log():
    if request.method == 'POST':
        db.session.add(MAR(
            mar_no=request.form['mar_no'],
            material=request.form['material'],
            supplier=request.form.get('supplier'),
            discipline=request.form.get('discipline'),
            status=request.form.get('status'),
            remarks=request.form.get('remarks'),
            signed_file_link=request.form.get('signed_file_link')
        ))
    ...
    if q:
        like = f'%{q}%'
        query = query.filter(db.or_(MAR.mar_no.ilike(like), MAR.material.ilike(like), MAR.supplier.ilike(like), MAR.discipline.ilike(like), MAR.status.ilike(like)))
    return render_template('mar.html', items=query.order_by(MAR.created_at.desc()).all(), q=q)

@app.route('/mar/<int:id>/delete', methods=['POST'])
@login_required
@roles_required('admin', 'dc')
def delete_mar(id):
    item = MAR.query.get_or_404(id)
    db.session.delete(item); db.session.commit()
    flash('MAR deleted.', 'success')
    return redirect(url_for('mar_log'))
...
def rfi_log():
    ...
    if q:
        like = f'%{q}%'
        query = query.filter(db.or_(RFI.rfi_no.ilike(like), RFI.subject.ilike(like), RFI.location.ilike(like), RFI.question.ilike(like), RFI.answer.ilike(like), RFI.status.ilike(like)))
    return render_template('rfi.html', items=query.order_by(RFI.created_at.desc()).all(), q=q)

@app.route('/rfi/<int:id>/delete', methods=['POST'])
@login_required
@roles_required('admin', 'dc')
def delete_rfi(id):
    item = RFI.query.get_or_404(id)
    db.session.delete(item); db.session.commit()
    flash('RFI deleted.', 'success')
    return redirect(url_for('rfi_log'))
...
def ncr_log():
    ...
    if q:
        like = f'%{q}%'
        query = query.filter(db.or_(NCR.ncr_no.ilike(like), NCR.description.ilike(like), NCR.location.ilike(like), NCR.root_cause.ilike(like), NCR.corrective_action.ilike(like), NCR.status.ilike(like)))
    return render_template('ncr.html', items=query.order_by(NCR.created_at.desc()).all(), q=q)

@app.route('/ncr/<int:id>/delete', methods=['POST'])
@login_required
@roles_required('admin', 'dc')
def delete_ncr(id):
    item = NCR.query.get_or_404(id)
    db.session.delete(item); db.session.commit()
    flash('NCR deleted.', 'success')
    return redirect(url_for('ncr_log'))

@app.route('/export/<name>')
@login_required
def export_log(name):
    if name == 'mar': return export_model(MAR.query.all(), 'MAR_Log', ['mar_no','material','supplier','discipline','status','remarks','signed_file_link','created_at'])
    if name == 'rfi': return export_model(RFI.query.all(), 'RFI_Log', ['rfi_no','subject','location','question','answer','status','signed_file_link','created_at'])
    if name == 'ncr': return export_model(NCR.query.all(), 'NCR_Log', ['ncr_no','description','location','root_cause','corrective_action','status','signed_file_link','created_at'])
    return redirect(url_for('dashboard'))
...
# -------------------- Simple database migration for existing Render DB --------------------
def ensure_column(table_name, column_name, column_type):
    """Add a missing column safely for existing SQLite/PostgreSQL databases."""
    try:
        ...
        print(f'Migration warning for {table_name}.{column_name}: {exc}')

def run_light_migrations():
    ensure_column('mar', 'signed_file_link', 'VARCHAR(500)')
    ensure_column('rfi', 'signed_file_link', 'VARCHAR(500)')
    ensure_column('ncr', 'signed_file_link', 'VARCHAR(500)')

# -------------------- Init --------------------
def upsert_default_user(username, full_name, password, role):
    user = User.query.filter_by(username=username).first()
    reset_passwords = os.environ.get('RESET_DEFAULT_USERS', 'false').lower() == 'true'
    if user:
        ...
        db.session.add(User(username=username, full_name=full_name, password_hash=generate_password_hash(password), role=role))

def seed():
    # Creates missing default users on first deployment.
    # If you need to reset default passwords on Render, set RESET_DEFAULT_USERS=true once, deploy, then set it back to false.
    upsert_default_user('admin', 'System Admin', 'admin123', 'admin')
    upsert_default_user('dc', 'Document Controller', 'dc123', 'dc')
    upsert_default_user('reviewer', 'Consultant Reviewer', 'review123', 'reviewer')
    db.session.commit()

with app.app_context():
    db.create_all(); run_light_migrations(); seed()
